...
int e_po6030k_config_cam(unsigned int sensor_x1,unsigned int sensor_y1,
	...
	//if(e_po6030k_set_wx(sensor_x1,(ARRAY_ORIGINE_X + ARRAY_WIDTH + 1)))
		return -1;

	if(e_po6030k_set_wy(sensor_y1 /zoom_sample,(ARRAY_ORIGINE_Y + ARRAY_HEIGHT + 1)/zoom_sample))
	//if(e_po6030k_set_wy(sensor_y1,(ARRAY_ORIGINE_Y + ARRAY_HEIGHT + 1)))
		return -1;
	...
	if(color_mode == GREY_SCALE_MODE) {
		switch (sampl_mode) {
			//clock divisors: 4, 2, 1 to have the same framerate => 
			//14'745'600 (camera clock) / ((800 (frame width) x 500 (frame height))) / 4 (divisor) = 9.216 fps 
		...
		switch (sampl_mode) {
			//clock divisors: 8, 4, 2 to have the same framerate => 
			//14'745'600 (camera clock) / ((800 (frame width) x 500 (frame height))) / 8 (divisor) = 4.608 fps 
			case PO_6030_MODE_VGA:
