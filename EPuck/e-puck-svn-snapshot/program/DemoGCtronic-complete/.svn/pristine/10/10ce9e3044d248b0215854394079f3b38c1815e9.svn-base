...
int e_acc_scan[3][ACC_SAMP_NB] = {{0},{0},{0}};			/*!< Array to store the acc values */
...
void e_init_ad_scan(unsigned char only_micro)
{
	...
	ADCSSLbits.CSSL0 = 0;   // ir8 on gumstix ext.
	ADCSSLbits.CSSL1 = 0;   // ir9 on gumstix ext.
		...
		{	
			...
			if(selector == 10) { // gumstix ext. demo
				if(prox_number < 4) {
					...
					ADCON2bits.SMPI = 3-1;	//interupt on 2th sample
