...
int e_po3030k_config_cam(unsigned int sensor_x1,unsigned int sensor_y1,
	...
	if(color_mode == GREY_SCALE_MODE) {
		switch (sampl_mode) {
			case MODE_VGA:
				e_po3030k_set_speed(SPEED_8);	//internal clock = 1/8 of the external clock
				break;
			case MODE_QVGA:
				e_po3030k_set_speed(SPEED_4);	//internal clock = 1/4 of the external clock
				break;
			case MODE_QQVGA:
				e_po3030k_set_speed(SPEED_2);	//internal clock = 1/2 of the external clock
				break;
		}
	} else {
		switch (sampl_mode) {
			case MODE_VGA:
				e_po3030k_set_speed(SPEED_16);	//internal clock = 1/16 of the external clock
				break;
			case MODE_QVGA:
